...
def run():
    ...
    def local_phase(v,spins,K,h,newspins):
        ...
        z=np.vdot(U,Tp);leak=float(np.linalg.norm(Tp-z*U));max_local_leak=max(max_local_leak,leak);max_phase_mod=max(max_phase_mod,abs(abs(z)-1))
    ...
    for h in H:
        ...
        neigh=all(map_node(D.neighbor[(v,r)],h)==D.neighbor[(map_node(v,h),h[r])] for v in range(16) for r in range(4))
        struct.append({'permutation':list(h),'parity':parity(h),'neighbor_slot_covariance':neigh,'dual_edge_orientation_reversals':rev,'edge_map_bijective':len(set(emap))==len(edges)})
    ...
    for h in H:
        ...
        seed_ok &= set(sm)=={seed} and abs(z-1)<TOL
    ...
    E=G.H_sine_basis(seed,0,5,TOL);en=G.norm(E);rows=[];maxerr=0.0;support=True;normdef=0.0
    for h in H:
        mapped=map_state(E,h);target={k:parity(h)*a for k,a in E.items()};err=relerr(mapped,target);maxerr=max(maxerr,err);support &= set(mapped)==set(target);normdef=max(normdef,abs(G.norm(mapped)-en))
        rows.append({'permutation':list(h),'parity':parity(h),'support_identical':set(mapped)==set(target),'relative_E_covariance_error':err,'mapped_norm':G.norm(mapped)})
    checks={
      'H_order8':len(H)==8,
      'graph_neighbor_slot_covariance':all(x['neighbor_slot_covariance'] for x in struct),
      'coordinate_permutations_preserve_edge_orientation':all(x['dual_edge_orientation_reversals']==0 for x in struct),
      'edge_maps_bijective':all(x['edge_map_bijective'] for x in struct),
      'global_K0_seed_invariant':bool(seed_ok),
      'local_K_line_recoupling_exact':max_local_leak<TOL and max_phase_mod<TOL,
      'E_sparse_support_covariant':bool(support),
      'E_pseudoscalar_amplitude_covariance':maxerr<TOL,
      'E_norm_preserved':normdef<TOL,
    }
    return {'status':'exact 16-cell Peter-Weyl covariance under K0 pairing stabilizer H','passed':bool(all(checks.values())),'science_status':'AMPLITUDE_ORBIT_REDUCTION_PREREQUISITE',
      'group':'H=(S2 x S2) semidirect S2 preserving pairing (01)(23)','order':len(H),'elements':[list(h) for h in H],
      'checks':checks,'structural_rows':struct,'seed_rows':seed_rows,'E_source0_support':len(E),'E_source0_norm':en,'E_covariance_rows':rows,
      'max_local_intertwiner_line_leakage':max_local_leak,'max_local_phase_modulus_defect':max_phase_mod,'max_E_relative_covariance_error':maxerr,'max_E_norm_defect':normdef,
