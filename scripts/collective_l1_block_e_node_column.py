...
def encode(key,amp):return {'spins':list(key[0]),'Ks':list(key[1]),'re':float(amp.real),'im':float(amp.imag)}
def run(local_index,parent_id=0):
    ...
    if not 0<=local_index<24:raise ValueError(local_index)
    v=inside[local_index];seed=((1,)*len(G.EDGES),(0,)*D.n_tets);col=G.H_sine_basis(seed,v,JMAX2);n=G.norm(col)
    changed=Counter(sum(s!=1 for s in key[0]) for key in col);par=Counter(sum(key[0])%2 for key in col)
    # Identify the block boundary dual links for later projection diagnostics.
    ins=set(inside);boundary=[e for e in G.EDGES if (e[0] in ins)^(e[1] in ins)];bei={e:i for i,e in enumerate(boundary)}
    patterns=Counter()
    for key in col:
        patterns[tuple(key[0][G.edge_index[e]] for e in boundary)]+=1
    ...
    checks={'L1_nodes_384':D.n_tets==384,'L1_dual_links_768':len(G.EDGES)==768,'parent_has_24_fine_nodes':len(inside)==24,
            'parent_boundary_has_24_dual_links':len(boundary)==24,'column_nonzero':len(col)>0 and n>1e-12,
            'finite_amplitudes':finite,'one_E_spin_wall':max(max(k[0]) for k in col)<=2}
    return {'status':'exact L1 barycentric fine-node Euclidean amplitude column','passed':bool(all(checks.values())),'science_status':'L1_BLOCK_E_PRECURSOR',
            'parent_coarse_tetra':parent_id,'local_fine_index':local_index,'global_fine_node':v,'L1_nodes':D.n_tets,'L1_dual_links':len(G.EDGES),
            'parent_fine_nodes':inside,'parent_boundary_dual_links':[list(e) for e in boundary],
            'support':len(col),'norm':n,'changed_edge_count_distribution':{str(k):v for k,v in sorted(changed.items())},
            'sum_doubled_spin_parity_distribution':{str(k):v for k,v in sorted(par.items())},
            'distinct_boundary_spin_patterns':len(patterns),'boundary_pattern_multiplicities':sorted(patterns.values()),
            'checks':checks,'seed':{'spins':list(seed[0]),'Ks':list(seed[1])},
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--local-index',type=int,choices=range(24),required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args()
    try:o=run(a.local_index);code=0 if o['passed'] else 1
    except Exception as exc:o={'status':'worker exception','passed':False,'science_status':'INFRASTRUCTURE_DIAGNOSTIC','local_fine_index':a.local_index,'error_type':type(exc).__name__,'error':str(exc),'traceback':traceback.format_exc()};code=1
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(o,indent=2)+'\n',encoding='utf-8');print(json.dumps({k:v for k,v in o.items() if k!='column'},indent=2));return code
