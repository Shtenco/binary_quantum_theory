...
geometry gives y=J h, hence q=B^{-1}J h. This prevents hidden trace-vs-traceless
normalization from fitting the DeWitt coefficient.
"""
...
def analyze(C,C00,B):
    ...
    Kh=Q.T@Kq@Q;Kh=.5*(Kh+Kh.T)
    lam_tr=float(TRACE@Kh@TRACE);Ktl=TL.T@Kh@TL;mix=TL.T@Kh@TRACE
    eig=np.linalg.eigvalsh(Ktl);lam_tl=float(np.trace(Ktl)/5);scale=max(abs(lam_tl),np.linalg.norm(Ktl)/math.sqrt(5),1e-300)
    anis=float(np.linalg.norm(Ktl-lam_tl*np.eye(5))/scale);mixing=float(np.linalg.norm(mix)/scale)
...
def selftest():
    A=1.7;c=.5;Kh=2*A*(np.eye(6)-3*c*np.outer(TRACE,TRACE));u=np.ones(6)/math.sqrt(6);Ptr=np.outer(u,u);Ptl=np.eye(6)-Ptr;B=1.3*Ptr+.7*Ptl;Q=np.linalg.solve(B,J);Kq=np.linalg.solve(Q.T,Kh)@np.linalg.inv(Q);C00=.23;C=.5*Kq+C00*np.eye(6);o=analyze(C,C00,B)
