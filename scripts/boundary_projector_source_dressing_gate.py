...
def run() -> dict:
    ...
    return {
        ...
        "exact_whitened_observables": {"X": OXbar0.tolist(), "Z": OZbar0.tolist()},
        "heat_kernel_staircase": rows,
        "finite_source_check": {"jx": jx, "jz": jz, "radius": float(R), "Z_expected_cosh": exact_Z, "Z_numerical": numerical_Z, "W_expected_log_cosh": exact_W, "W_numerical": numerical_W},
        "checks": checks,
        "interpretation": "A strictly positive boundary-compressed master can coexist with nonzero boundary overlap with the full zero projector. The legal source order is full master -> zero projector/heat filter -> boundary Gram and projected observable -> support whitening -> Z[J].",
        "production_target": "Replace the 4D control by the 32-column q=2 boundary block and the parity-complete Peter-Weyl master; evaluate G_tau and source matrices with block Krylov rather than a dense ambient diagonalization.",
        "claim_boundary": "No BQG physical zero mode, physical clock, cosmological vacuum, dark matter, dark energy, lensing signal or physical frequency is established by this control.",
    }
